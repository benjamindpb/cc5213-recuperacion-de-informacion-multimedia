    ...
    def leer_archivo_gt(self, filename):
        ...
        with open(filename) as f:
            for linea in f:
                ...
                if partes[0] not in ["Q", "C", "G", "F"]:
                    raise Exception("{}: archivo GT invalido (tipos incorrectos)".format(filename))
...
nombres_tipos = dict()
nombres_tipos["Q"] = "QUALITY"
nombres_tipos["C"] = "CROP"
nombres_tipos["G"] = "GAMMA"
nombres_tipos["F"] = "FLIP"
...
class Evaluador:
    ...
    def imprimir_resultado(self):
        ...
        recall_7 = 0.75
        if self.metricas.recall < recall_7:
            nota_calidad = round(7 * self.metricas.recall / recall_7, 1)
            bonus = 0
        else:
            nota_calidad = 7
            bonus = round((self.metricas.recall - recall_7) / (1 - recall_7), 1)
        if nota_calidad < 1:
            nota_calidad = 1
...
def evaluar_resultados(filename_gt, filename_resultados):
...
# inicio
if len(sys.argv) < 3:
    print("CC5213 - Evaluación Tarea 1  (2021-1)")
    print("Uso: {} [filename_gt] [filename_resultados]".format(sys.argv[0]))
    sys.exit(1)

filename_gt = sys.argv[1]
filename_resultados = sys.argv[2]

evaluar_resultados(filename_gt, filename_resultados)
